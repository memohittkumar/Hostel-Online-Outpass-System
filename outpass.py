...
root.geometry("940x580+300+70")
root.resizable(False, False)
root.title('Welcome')


photo=PhotoImage(file='C:\\Users\\HP Laptop\\PycharmProjects\\untitled\\image\\jainuni.png')
...
def signup():
    ...
    def insert():
        ...
        c.execute('INSERT INTO details (s_name, u_sn, pass_word)  VALUES (?,?,?)', (sname, usn, password,))
        conn.commit()
        conn.close()
    ...
    Label(s, text='USN :', bg='#D0ECE7', font=('Bookman Old Style', 15, 'bold')).place(x=60, y=190)
...
Label(root, text=' WELCOME TO ONLINE OUTPASS ', bg='#083C41', fg='white', font=('Comic Sans MS', 40, 'bold')).place(x=0, y=0)
Label(root,text='USERID :', bg='#7F8C8D' ,font=('Bookman Old Style',15,'bold')).place(x=260,y=170)
Label(root,text='PASSWORD :',bg='#7F8C8D',font=('Bookman Old Style',15,'bold')).place(x=260,y=220)
e1=Entry(root, width=20,font=('Verdana', 15))
e1.place(x=415,y=170)
e2=Entry(root,show='*',width=20,font=('Verdana', 15))
e2.place(x=415,y=220)
Button(root,text='SIGNUP',width=10,command=signup,font=('Bookman Old Style', 15)).place(x=500,y=290)
wname = e1.get()
passw = e2.get()
...
def login():
    ...
    conn = sqlite3.connect('Main.db')
    c = conn.cursor()
    la = Label(f, text='       DETAILS         ', bg='#083C41', fg='white',font=('Comic Sans MS', 40, 'bold')).place(x=0, y=0)
    ...
    Sb6 = Spinbox(f, from_=2019, to=2030, width=5)
    ...
    '''def inserts():
        ...
        c.execute('CREATE TABLE IF NOT EXISTS form(FOREIGN KEY(id) REFERENCES details(u_sn) , room NUMBER, y_ear NUMBER, place TEXT,phone NUMBER,purpose TEXT,d_to NUMBER,d_from NUMBER,t_to TEXT,t_from TEXT)')
        c.execute('INSERT INTO details (id,room,y_ear,place,phone,purpose,d_to,d_from,t_to,t_from)  VALUES (?,?,?,?,?,?,?,?,?,?)',(i_d, roomno, year, pl_ace, phoneno, pur_pose, date_to, date_from, time_to, time_from))
        conn.commit()
        conn.close()

    Button(f, text='SUBMIT',command=insert, width=10, font=('Bookman Old Style', 14)).place(x=210, y=560)'''
    f.mainloop()
...
'''def msg():
        msg = 'invalid username or password! Retry'
        messageVar = Message(root, text=msg)
        messageVar.place(x=430,y=350)'''

def query():
      wname = e1.get()
      passw = e2.get()
      c.execute("SELECT * FROM warden WHERE s_name = ? and pass_word = ?", (wname,passw))
      war = c.fetchall()
      #print(war)
      sname = e1.get()
      password = e2.get()
      c.execute("SELECT * FROM details WHERE s_name = ? and pass_word = ?", (sname,password))
      records = c.fetchall()
...
Button(root,text='LOGIN',width=10,command=query,font=('Bookman Old Style', 15)).place(x=320,y=290)
